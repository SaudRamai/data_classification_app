...
with tab_inv_browser:
    ...
    try:
        ...
        t_assets = int(kpi_data_view.get('Total_Assets', 0) or 0)
        t_classified = int(kpi_data_view.get('Classified', 0) or 0)
        cov_display_raw = kpi_data_view.get('Classification_Coverage', '0% ❌')
        cov_display = cov_display_raw if cov_display_raw else '0% ❌'
        # Safely extract formatting from the emoji-included coverage string if needed
        ...
        viol_count = _get_policy_violations(active_db)
...
with tab_inv_browser:
    ...
    b_db, b_schema, b_tbl, b_pii, b_sox, b_soc2, b_cols = st.tabs([
        "🏛️ Database View", 
        "📚 Schema View", 
        "📑 Table/View List", 
        "👤 PII Assets Only",
    ...
    with b_db:
        ...
        if db_df.empty:
            ...
            show_cols = ["Database", "Total_Assets", "Classified", "Classification_Coverage", "Compliance_Status", "Sensitive_Columns"]
    ...
    with b_schema:
        ...
        if sch_df.empty:
            ...
            show_cols = ["Database", "Schema", "Total_Assets", "Classified", "Classification_Coverage", "Compliance_Status", "Sensitive_Columns"]
    ...
    with b_tbl:
        ...
        if tbl_df.empty:
            ...
            if tbl_df.empty:
                st.info("No Table or View assets found matching the criteria.")
            else:
                show_cols = ["Schema", "Asset_Name", "Asset_Type", "Classification_Coverage", "Compliance_Status", "Column_Stats", "Sensitive_Columns"]
    ...
    # SOX Assets Tab
    with b_sox:
        st.caption("Assets marked as SOX relevant in the governance ASSETS table.")

        sox_filters = st.session_state.get("global_filters", {})
        try:
            db_name = active_db
            if not db_name:
                st.info("Please select a database to view SOX assets.")
            else:
                conds = [
                ...
                if sox_filters.get("schema") and sox_filters["schema"] != "All":
                    conds.append(f"SCHEMA_NAME = '{sox_filters['schema']}'")
                if sox_filters.get("table") and sox_filters["table"] != "All":
                    conds.append(f"ASSET_NAME = '{sox_filters['table']}'")
                ...
                """
                sox_rows = snowflake_connector.execute_query(query) or []
                if not sox_rows:
                    st.info("No SOX-relevant assets found for the current filters.")
                else:
                    sox_df = pd.DataFrame(sox_rows)
                    st.dataframe(sox_df, width='stretch', hide_index=True)

                    if st.button("Export SOX Assets", key="btn_exp_sox"):
                        try:
                            csv = sox_df.to_csv(index=False).encode('utf-8')
                            st.download_button("Download CSV", data=csv, file_name="sox_assets.csv", mime="text/csv")
                        except Exception:
                            pass
        except Exception as e:
            logger.error(f"Failed to load SOX assets: {e}")
            st.error(f"Failed to load SOX assets: {e}")

    # SOC2 Assets Tab
    with b_soc2:
        st.caption("Assets marked as SOC2 relevant in the governance ASSETS table.")

        soc_filters = st.session_state.get("global_filters", {})
        try:
            db_name = active_db
            if not db_name:
                st.info("Please select a database to view SOC2 assets.")
            else:
                conds = [
                    f"DATABASE_NAME = '{db_name}'",
                    "UPPER(ASSET_TYPE) IN ('TABLE', 'VIEW', 'BASE TABLE', 'STORED_PROCEDURE', 'FUNCTION', 'STAGE', 'DATABASE')",
                ...
                if soc_filters.get("schema") and soc_filters["schema"] != "All":
                    conds.append(f"SCHEMA_NAME = '{soc_filters['schema']}'")
                if soc_filters.get("table") and soc_filters["table"] != "All":
                    conds.append(f"ASSET_NAME = '{soc_filters['table']}'")
                ...
                query = f"""
                    ...
                    ORDER BY SCHEMA_NAME, ASSET_NAME
                """
                soc_rows = snowflake_connector.execute_query(query) or []
                if not soc_rows:
                    st.info("No SOC2-relevant assets found for the current filters.")
                else:
                    soc_df = pd.DataFrame(soc_rows)
                    st.dataframe(soc_df, width='stretch', hide_index=True)

                    if st.button("Export SOC2 Assets", key="btn_exp_soc2"):
                        try:
                            csv = soc_df.to_csv(index=False).encode('utf-8')
                            st.download_button("Download CSV", data=csv, file_name="soc2_assets.csv", mime="text/csv")
                        except Exception:
                            pass
        except Exception as e:
            logger.error(f"Failed to load SOC2 assets: {e}")
            st.error(f"Failed to load SOC2 assets: {e}")
    ...
    with b_cols:
        ...
        if col_df.empty:
            ...
            st.dataframe(
                v_df[["Column Name", "Table Name", "AI Category", "Native Recommendation", "Masking Status", "Sensitivity Level", "PII Flag"]], 
                width='stretch', 
                hide_index=True,
                column_config={
    ...
    with tab_favorites:
            ...
            for dbn, fns in by_db.items():
                try:
                    ...
                    rows = snowflake_connector.execute_query(
                        ...
                        FROM {dbn}.DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_DECISIONS
        ...
        if 'sel_asset' in locals() and sel_asset and not pd.isna(sel_asset):
            ...
            try:
                ...
                if not db_ctx or str(db_ctx).strip().upper() in {"", "NONE", "(NONE)", "NULL", "UNKNOWN"}:
                    db_ctx = "DATA_CLASSIFICATION_DB"
                ...
                hist = snowflake_connector.execute_query(
                    ...
                    FROM {db_ctx}.DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_DECISIONS
                ...
                try:
                    hist2 = snowflake_connector.execute_query(
                        ...
                        FROM {db_ctx}.DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_DECISIONS_HISTORY
                        WHERE ASSET_FULL_NAME = %(asset)s
