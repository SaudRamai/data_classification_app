...
def render_realtime_dashboard():
    ...
    if not _db_raw or str(_db_raw).strip().upper() in ("NONE", "NULL", "(NONE)", "UNKNOWN", ""):
        _db_raw = getattr(settings, "SNOWFLAKE_DATABASE", None)
    ...
    try:
        ...
        # Inject CSS to style the column container div (targets stVerticalBlock inside columns)
        st.markdown("""
            <style>
        ...
        with col_gauge:
            ...
            # Gauge — number rendered as a centred annotation inside the arc
            fig = go.Figure(go.Indicator(
                mode = "gauge",           # no "+number" — we draw it ourselves
                value = score,
                domain = {'x': [0, 1], 'y': [0, 1]},
                ...
                gauge = {
                    ...
                    'steps': [
                        ...
                        {'range': [40, 75], 'color': 'rgba(241, 196, 15, 0.12)'},
            ...
            # Place the score number at the centre of the semicircle hollow
            fig.add_annotation(
                x=0.5, y=0.18,
                text=f"<b>{score}%</b>",
        ...
        with col_info:
            ...
            if score < 40:
                rec_icon = "🔴"
                rec_title = "Critical"
                rec_body = "High volume of unclassified assets or SLA breaches. Priority: Bulk classification scan."
                rec_color = "#fc8181"
            elif score < 75:
                rec_icon = "🟠"
                rec_title = "Needs Attention"
                rec_body = "Governance workflow is lagging. Priority: Approve pending labels and sync reviews."
                rec_color = "#f6ad55"
            else:
                rec_icon = "🟢"
                rec_title = "Healthy"
                rec_body = "Program meeting targets. Priority: Regular drift monitoring and policy refinement."
                rec_color = "#68d391"

            total_assets = health.get('total_assets', 0)
            classified_count = health.get('classified_count', round(total_assets * health.get('coverage_pct', 0) / 100))
            pending_count = total_assets - classified_count
            cov_pct = health.get('coverage_pct', 0)
            acc_pct = health.get('approval_pct', 0)
            tim_pct = health.get('sla_pct', 0)
            gov_pct = health.get('reviews_pct', 0)

            def _pct_emoji(v):
                return "✅" if v >= 90 else "☑️" if v >= 70 else "⚠️" if v >= 50 else "❌"

            def _ind_color(v):
                return "#68d391" if v >= 90 else "#4fd1c5" if v >= 70 else "#f6ad55" if v >= 50 else "#fc8181"
            ...
            st.markdown(f"""
                <div class="info-panel">
                    ...
                    <div class="info-item">
                        ...
                        <span><b style="color:#fff;font-size:15px">{total_assets:,}</b>&nbsp;Total active assets included</span>
                    </div>
                    <div class="info-item">
                        <span class="info-bullet">▹</span>
                        <span>
                            <b style="color:#68d391;font-size:15px">{classified_count:,}</b>&nbsp;classified
                            &nbsp;<span style="color:rgba(255,255,255,0.3)">|</span>&nbsp;
                            <b style="color:#fc8181;font-size:15px">{pending_count:,}</b>&nbsp;pending
                        </span>
                    </div>
                    <div class="info-title" style="margin-top:15px">Actionable Insight</div>
                    <div style="font-size:13px; color:rgba(255,255,255,0.9); line-height:1.7; padding:8px 10px; background:rgba(255,255,255,0.03); border-left:3px solid {rec_color}; border-radius:4px;">
                        {rec_icon}&nbsp;<strong style="color:{rec_color}">{rec_title}:</strong>&nbsp;{rec_body}
                    </div>
                    <div class="info-title" style="margin-top:15px">Key Indicators</div>
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-top:6px;">
                        <div class="info-item">
                            <span style="font-size:14px">{_pct_emoji(cov_pct)}</span>&nbsp;
                            <span style="font-size:12px; color:rgba(255,255,255,0.6)">Coverage</span>&nbsp;
                            <b style="color:{_ind_color(cov_pct)};font-size:13px">{cov_pct}%</b>
                        </div>
                        <div class="info-item">
                            <span style="font-size:14px">{_pct_emoji(acc_pct)}</span>&nbsp;
                            <span style="font-size:12px; color:rgba(255,255,255,0.6)">Accuracy</span>&nbsp;
                            <b style="color:{_ind_color(acc_pct)};font-size:13px">{acc_pct}%</b>
                        </div>
                        <div class="info-item">
                            <span style="font-size:14px">{_pct_emoji(tim_pct)}</span>&nbsp;
                            <span style="font-size:12px; color:rgba(255,255,255,0.6)">Timeliness</span>&nbsp;
                            <b style="color:{_ind_color(tim_pct)};font-size:13px">{tim_pct}%</b>
                        </div>
                        <div class="info-item">
                            <span style="font-size:14px">{_pct_emoji(gov_pct)}</span>&nbsp;
                            <span style="font-size:12px; color:rgba(255,255,255,0.6)">Governance</span>&nbsp;
                            <b style="color:{_ind_color(gov_pct)};font-size:13px">{gov_pct}%</b>
                        </div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
    ...
    try:
        ...
        labels_map = sens_data.get('labels') or {}
        # drop non-positive counts to avoid blank charts
        labels_map = {k: int(v) for k, v in labels_map.items() if v is not None and int(v) > 0}
        _sens_fqn = T_ASSETS
        ...
        with c_s1:
            ...
            if labels_map:
                df_labels = pd.DataFrame(list(labels_map.items()), columns=['Label', 'Count'])
                fig_labels = px.pie(
                ...
                if total_assets > 0:
                    st.info("No classification labels found. Showing unclassified-only scope.")
                else:
                    st.info("No assets found for this scope.")
        ...
        # Lightweight diagnostics when the section is effectively empty
        try:
            if total_assets == 0:
                diag_row = (_rt_run_query(
                    f"""
                    SELECT
                        COUNT(*) AS TOTAL,
    ...
    try:
        ...
        exception_count = 0
        try:
            ex_row = (_rt_run_query(
                f"""
                SELECT COUNT(*) AS C
                FROM {T_ASSETS}
                WHERE COALESCE(HAS_EXCEPTION, FALSE) = TRUE
                  AND DATABASE_NAME IS NOT NULL
                """
            ) or [{}])[0]
            exception_count = int((ex_row or {}).get("C") or 0)
        except Exception:
            exception_count = 0
        ...
        with c6:
            readiness = comp_data.get('audit_readiness', 'HIGH')
            readiness_color = "#28A745" if readiness == "HIGH" else "#F1C40F" if readiness == "MEDIUM" else "#E74C3C"
            st.markdown(f"""
                <div class="pillar-card" style="border-top: 4px solid {readiness_color};">
                    <div class="pillar-icon">✅</div>
                    <div class="pillar-label">Audit Readiness</div>
                    <div class="pillar-value" style="font-size: 22px;">{readiness}</div>
        ...
        try:
             ...
             asset_rows = _rt_run_query(f"""
                ...
                where UPPER(ASSET_TYPE) IN ('TABLE', 'VIEW', 'BASE TABLE')
                  AND LAST_MODIFIED_TIMESTAMP >= dateadd('day', -7, current_timestamp())
                order by TS desc
             """)
