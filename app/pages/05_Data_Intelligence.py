...
from src.services.asset_catalog_service import get_asset_counts
...
with q_tab:
    ...
    with dq_dash:
        ...
        try:
            from src.services.asset_catalog_service import get_sensitivity_overview, get_asset_counts
...
""", unsafe_allow_html=True)
        ...
        with s_col1:
            ...
            try:
                ...
                db = st.session_state.get('sf_database') or _active_db_from_filter() if '_active_db_from_filter' in globals() else None
                if not db:
                    # Fallback to current database
                    try:
                        result = snowflake_connector.execute_query("SELECT CURRENT_DATABASE() as DB")
                        db = result[0].get('DB') if result else "DATA_CLASSIFICATION_DB"
                    except Exception:
                ...
                if rows_inc:
                    st.dataframe(pd.DataFrame(rows_inc), width='stretch', hide_index=True)
                else:
                ...
                with res_tab2:
                    ...
                    try:
                        ...
                        if not sample_full.empty:
                            st.dataframe(sample_full, width='stretch', hide_index=True)
                        else:
                            st.info("Empty asset.")
                    except Exception as e:
    ...
    with dq_issues:
        ...
        with st.container(border=True):
            ...
            with c_opt3:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🚀 Refresh Quality Scan", type="secondary", width='stretch'):
                    st.cache_data.clear()
...
# ... (rest of the code remains the same)
        ...
        with i_tab3:
            if rows_struc:
                ...
                st.dataframe(df_struc, width='stretch', hide_index=True)
            else:
