...
class SnowflakeConnector:
    ...
    def _get_cached_connection(self):
        ...
        # Check session state first (Streamlit-based user session)
        if st is not None:
            acct = st.session_state.get("sf_account")
            user = st.session_state.get("sf_user")
            pwd = st.session_state.get("sf_password")
            ...
            if acct and user:
                ...
                if db and str(db).strip().upper() not in ("NONE", "NULL", "(NONE)", "UNKNOWN", ""):
                    params["database"] = db
                if sc: params["schema"] = sc
                if role: params["role"] = role
                if organization: params["organization"] = str(organization).strip()
                if host_override:
                    try:
                        h = str(host_override).strip().lower()
                        if h.startswith("http://"): h = h[len("http://"):]
    ...
    def execute_query(self, query: str, params: Optional[Dict[str, Any]] = None, silent: bool = False) -> list:
        """
        ...
        try:
            ...
            # Additional safety: ensure we aren't about to run a query that references a 'NONE' database
            if "NONE." in query.upper() or query.strip().upper().startswith("USE DATABASE NONE"):
                logger.warning(f"Intercepted query with invalid database reference: {query[:100]}...")
                return []
    ...
    def execute_non_query(self, query: str, params: Optional[Dict[str, Any]] = None, silent: bool = False) -> int:
        """
        ...
        try:
            ...
            # Additional safety
            if "NONE." in query.upper() or query.strip().upper().startswith("USE DATABASE NONE"):
                logger.warning(f"Intercepted statement with invalid database reference: {query[:100]}...")
                return 0
