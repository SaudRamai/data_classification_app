...
# Cached governance metadata loader - runs ONCE per session
@st.cache_resource(show_spinner="Loading governance metadata...")
def load_governance_metadata_once():
    """
    Load and cache all governance metadata once per Streamlit session.
    Returns a dict with all loaded metadata to prevent per-column re-initialization.
    """
    logger = logging.getLogger(__name__)
    logger.info("=== LOADING GOVERNANCE METADATA (CACHED) ===")
    
    try:
        # Initialize the pipeline service instance
        from src.services.classification_pipeline_service import AIClassificationPipelineService
        temp_service = AIClassificationPipelineService()
        
        # Load all metadata in one go
        metadata = {
            'loaded_at': datetime.now().isoformat(),
            'classification_rules': [],
            'category_metadata': {},
            'exclusion_patterns': {'exact': [], 'regex': []},
            ...
            'use_direct_tables': False,
            'embed_ready': False
        }
        
        # Check if views exist or use direct tables
        schema_fqn = temp_service._resolve_governance_schema()
        use_direct_tables_key = "use_direct_tables"
        
        if use_direct_tables_key not in st.session_state:
            # Test if views exist
            try:
                test_view = snowflake_connector.execute_query(f"SELECT 1 FROM {schema_fqn}.VW_CATEGORY_METADATA LIMIT 1")
                st.session_state[use_direct_tables_key] = False
                logger.info("✓ Governance views available - using VW_* views")
            except Exception:
                st.session_state[use_direct_tables_key] = True
                logger.warning("⚠️ Governance views unavailable - falling back to direct tables (logged once)")
        
        metadata['use_direct_tables'] = st.session_state[use_direct_tables_key]
        
        # Load category metadata
        if metadata['use_direct_tables']:
            # Direct table access
            meta_rows = snowflake_connector.execute_query(f"""
                SELECT CATEGORY_ID, CATEGORY_NAME, COALESCE(DESCRIPTION,'') AS DESCRIPTION,
                       ...
                       COALESCE(MULTI_LABEL,TRUE) AS MULTI_LABEL, POLICY_GROUP
        ...
        for row in meta_rows:
            if isinstance(row, dict):
                ...
                if cat_name:
                    metadata['category_metadata'][cat_name] = {
                        'category_id': row.get('CATEGORY_ID'),
                        'description': str(row.get('DESCRIPTION') or ''),
                        'detection_threshold': float(row.get('DETECTION_THRESHOLD') or 0.45),
                        'weight_embedding': float(row.get('WEIGHT_EMBEDDING') or 0.60),
                        'weight_keyword': float(row.get('WEIGHT_KEYWORD') or 0.25),
                        'weight_pattern': float(row.get('WEIGHT_PATTERN') or 0.15),
                        'multi_label': bool(row.get('MULTI_LABEL', True)),
                        'policy_group': row.get('POLICY_GROUP')
                    }
        
        # Load classification rules (keywords/patterns)
        if metadata['use_direct_tables']:
            # Direct table access
            rules_rows = snowflake_connector.execute_query(f"""
                SELECT CATEGORY_NAME, RULE_TYPE, RULE_PATTERN,
                       COALESCE(RULE_WEIGHT,1.0) AS RULE_WEIGHT,
                       COALESCE(MATCH_TYPE,'CONTAINS') AS MATCH_TYPE
                FROM {schema_fqn}.SENSITIVE_KEYWORDS
                WHERE COALESCE(IS_ACTIVE, TRUE)
                UNION ALL
                SELECT CATEGORY_NAME, 'PATTERN' AS RULE_TYPE, PATTERN_VALUE AS RULE_PATTERN,
                       COALESCE(WEIGHT,1.0) AS RULE_WEIGHT,
                       'REGEX' AS MATCH_TYPE
        ...
        for row in rules_rows:
            if isinstance(row, dict):
                ...
                cat_name = str(row.get('CATEGORY_NAME') or '').strip()
                pattern = str(row.get('RULE_PATTERN') or '').strip()
                
                if cat_name and pattern:
                    if rule_type == 'KEYWORD':
                        if cat_name not in metadata['category_keywords']:
                            metadata['category_keywords'][cat_name] = []
                        metadata['category_keywords'][cat_name].append(pattern.lower())
                    elif rule_type == 'PATTERN':
                        if cat_name not in metadata['category_patterns']:
                            metadata['category_patterns'][cat_name] = []
                        metadata['category_patterns'][cat_name].append(pattern)
        
        # Load exclusion patterns - fix to use VW_EXCLUSION_PATTERNS view
        exclusion_cache_key = "exclusion_patterns_loaded"
        if exclusion_cache_key not in st.session_state:
            try:
                if metadata['use_direct_tables']:
                    # Try direct table first
                    excl_data = snowflake_connector.execute_query(f"""
                        SELECT PATTERN_TYPE, PATTERN_VALUE, EXCLUSION_REASON
                    ...
                    excl_data = snowflake_connector.execute_query(f"""
                        SELECT PATTERN_TYPE, PATTERN_VALUE, EXCLUSION_REASON
                        FROM {schema_fqn}.VW_EXCLUSION_PATTERNS
                        WHERE COALESCE(IS_ACTIVE, TRUE)
                        ORDER BY PATTERN_TYPE, PATTERN_VALUE
                    """) or []
                ...
                for excl in excl_data:
                    if isinstance(excl, dict):
                        ...
                        if pattern_type == 'COLUMN_NAME' and pattern_value:
                            metadata['exclusion_patterns']['exact'].append(pattern_value.lower())
                        elif pattern_type == 'REGEX' and pattern_value:
                            metadata['exclusion_patterns']['regex'].append({
                                'pattern': pattern_value,
                                'reason': excl.get('EXCLUSION_REASON', 'Regex exclusion')
                            })
                
                st.session_state[exclusion_cache_key] = True
                logger.info(f"✓ Loaded {len(metadata['exclusion_patterns']['exact'])} exact and {len(metadata['exclusion_patterns']['regex'])} regex exclusions")
                
            except Exception as e:
                logger.warning(f"⚠️ Failed to load exclusion patterns: {e}")
                # Use default exclusions only once
                metadata['exclusion_patterns'] = {
                    'exact': ['id', 'uuid', 'guid', 'created_by', 'updated_by', 'created_at', 'updated_at', 'deleted_at', 'version'],
                    'regex': [
                ...
                st.session_state[exclusion_cache_key] = True
                logger.info("Using default exclusion patterns (cached)")
        
        # Check embedder availability once
        try:
            if SentenceTransformer is not None and np is not None:
                # Test loading a small model
                test_model = SentenceTransformer('all-MiniLM-L6-v2')
                test_vector = test_model.encode(['test'], normalize_embeddings=True)
                metadata['embedder_available'] = True
                logger.info("✓ SentenceTransformer embedder available")
            else:
                metadata['embedder_available'] = False
                logger.warning("⚠️ SentenceTransformer or NumPy unavailable - centroid scoring disabled")
        except Exception as e:
            metadata['embedder_available'] = False
            logger.warning(f"⚠️ AI embedder unavailable - centroid scoring disabled: {e}")
        
        # Generate centroids only if embedder is available
        if metadata['embedder_available']:
            logger.info("Generating category centroids...")
            try:
                model = SentenceTransformer('all-MiniLM-L6-v2')
                for cat_name, cat_meta in metadata['category_metadata'].items():
                    description = cat_meta.get('description', '')
                    keywords = metadata['category_keywords'].get(cat_name, [])
                    
                    # Build rich text for centroid
                    centroid_text = f"{cat_name}: {description}"
                    if keywords:
                        centroid_text += f" Keywords: {', '.join(keywords[:20])}"
                    
                    # Generate centroid with passage: prefix for E5 models
                    passage_text = f"passage: {centroid_text}"
                    centroid_vec = model.encode([passage_text], normalize_embeddings=True)[0]
                    metadata['category_centroids'][cat_name] = centroid_vec
                    
                    # Generate tokens
                    tokens = temp_service._generate_category_tokens(cat_name, centroid_text)
                    metadata['category_tokens'][cat_name] = tokens or []
                
                metadata['embed_ready'] = True
                logger.info(f"✓ Generated {len(metadata['category_centroids'])} category centroids")
                
            except Exception as e:
                logger.error(f"Failed to generate centroids: {e}")
                metadata['embed_ready'] = False
        else:
            logger.info("Skipping centroid generation - embedder unavailable")
            metadata['embed_ready'] = False
        
        # Store governance_loaded_at timestamp
        st.session_state['governance_loaded_at'] = metadata['loaded_at']
        
        logger.info(f"=== GOVERNANCE METADATA LOADED SUCCESSFULLY ===")
        logger.info(f"Categories: {len(metadata['category_metadata'])}")
        logger.info(f"Keywords: {sum(len(k) for k in metadata['category_keywords'].values())}")
        logger.info(f"Patterns: {sum(len(p) for p in metadata['category_patterns'].values())}")
        logger.info(f"Exclusions: {len(metadata['exclusion_patterns']['exact'])} exact, {len(metadata['exclusion_patterns']['regex'])} regex")
        logger.info(f"Embedder: {'Available' if metadata['embedder_available'] else 'Unavailable'}")
        logger.info(f"Centroids: {len(metadata['category_centroids'])}")
        
        return metadata
        
    except Exception as e:
        logger.error(f"Failed to load governance metadata: {e}")
...
class DiscoveryService:
    ...
    def scan(self, database: str = None, include_views: bool = True, limit: int = 1000, offset: int = 0) -> int:
        ...
        obj_types = ("'BASE TABLE', 'TABLE'" + (", 'VIEW'" if include_views else ""))
        rows = self.connector.execute_query(
            f"""
            SELECT 
        ...
        try:
            ...
            query = f"""
                SELECT 
                    ...
                    DATA_OWNER, 
                    CREATED_TIMESTAMP 
                FROM {db}.{self._schema}.CLASSIFICATION_QUEUE 
                LIMIT %(limit)s
            """
    ...
    def mark_classified(self, full_name: str, cls: str, c: int, i: int, a: int, compliance_frameworks: List[str] = None, special_categories: List[str] = None, user: str = "SYSTEM") -> None:
        self._ensure_tables_once()
        try:
            db = self._normalize_db()
            fw_list = [f.upper() for f in (compliance_frameworks or [])]
            sc_list = [s.upper() for s in (special_categories or [])]
            
            pii_rel = 'TRUE' if ('PII' in fw_list or 'PII' in sc_list) else 'FALSE'
            sox_rel = 'TRUE' if ('SOX' in fw_list or 'FINANCIAL' in sc_list) else 'FALSE'
            soc_rel = 'TRUE' if ('SOC' in fw_list or 'SOC2' in fw_list) else 'FALSE'
            ...
            self.connector.execute_non_query(
                ...
                SET CLASSIFICATION_LABEL = %(cls)s,
                    ...
                    CLASSIFIED_BY = %(user)s,
                    CLASSIFICATION_METHOD = 'MANUAL_APPROVAL',
                    LAST_MODIFIED_TIMESTAMP = CURRENT_TIMESTAMP,
                    COMPLIANCE_STATUS = 'COMPLIANT',
                    REVIEW_STATUS = 'Completed',
                    PII_RELEVANT = {pii_rel},
                    SOX_RELEVANT = {sox_rel},
                    SOC2_RELEVANT = {soc_rel}
                WHERE FULLY_QUALIFIED_NAME = %(full)s
                """,
                {"full": full_name, "cls": cls, "c": c, "i": i, "a": a, "user": user},
            )
        except Exception as e:
...
SHOW TABLES LIKE 'SENSITIVITY_CATEGORIES' IN DATABASE <YOUR_DATABASE>;
                ...
                with st.container():
                    ...
                    with c5:
                        # ── Auto-suggest Compliance Frameworks ─────────────────
                        _fw_allowed = ["PII", "SOX", "SOC"]
                        try:
                            from src.services.tagging_service import TAG_DEFINITIONS as _TD
                            _fw_allowed = list(_TD.get("COMPLIANCE_FRAMEWORKS", _fw_allowed)) or _fw_allowed
                        except Exception:
                            pass

                        _auto_fw: set = set()

                        # Signal 1: already-detected Compliance column in table_details
                        try:
                            _comp_col = "Compliance" if "Compliance" in table_details.columns else None
                            _cat_col  = "All Categories" if "All Categories" in table_details.columns else (
                                        "Primary Category" if "Primary Category" in table_details.columns else None)

                            _rows_to_scan = (
                                table_details[table_details["Column"] == target_col]
                                if (target_type == "Column" and target_col)
                                else table_details
                            )

                            _fw_keyword_map = {
                                "PII": ["PII", "PERSONAL", "GDPR", "CCPA", "PHI", "HIPAA", "PCI",
                                        "SSN", "EMAIL", "PHONE", "DOB", "ADDRESS", "PASSPORT",
                                        "AADHAAR", "PAN", "NATIONAL_ID", "EMPLOYEE", "CUSTOMER"],
                                "SOX": ["SOX", "FINANCIAL", "ACCOUNTING", "GAAP", "IFRS",
                                        "PAYROLL", "LEDGER", "GL", "REVENUE", "EXPENSE",
                                        "AUDIT", "INCOME", "EARNINGS", "BALANCE_SHEET"],
                                "SOC": ["SOC", "SOC2", "SOC1", "REGULATORY",
                                        "SECURITY_AUDIT", "SYSTEM_CONTROL", "COMPLIANCE_CONTROL"],
                            }

                            for _, _row in _rows_to_scan.iterrows():
                                _txt = " ".join([
                                    str(_row.get(_comp_col, "") if _comp_col else ""),
                                    str(_row.get(_cat_col, "")  if _cat_col  else ""),
                                ]).upper()
                                for _fw, _keys in _fw_keyword_map.items():
                                    if _fw in _fw_allowed and any(_k in _txt for _k in _keys):
                                        _auto_fw.add(_fw)
                        except Exception:
                            pass

                        # Signal 2: asset / table name heuristics
                        try:
                            _tname_up = str(selected_table or "").upper()
                            _name_map = {
                                ...
                                "SOX": ["SOX","GL","LEDGER","PAYROLL","REVENUE","EXPENSE",
                                        "FINANCIAL_REPORT","GAAP","IFRS","AUDIT","INCOME",
                                        "BALANCE_SHEET","EARNINGS"],
                                "SOC": ["SOC","SOC2","SOC1","SECURITY_AUDIT","SYSTEM_CONTROL"],
                            }
                            for _fw, _keys in _name_map.items():
                                if _fw in _fw_allowed and any(_k in _tname_up for _k in _keys):
                                    _auto_fw.add(_fw)
                        except Exception:
                            pass

                        _auto_fw_list = sorted(_auto_fw & set(_fw_allowed))

                        # ── Seed session state so auto-detected values appear as
                        #    selected chips inside the dropdown.
                        #    Use a dynamic key per (table, column) so every new
                        #    selection resets with fresh suggestions.
                        _fw_key = f"tag_compliance__{hash(str(selected_table))}_{hash(str(target_col))}"
                        # Seed if: first render, OR previously cached as empty but we now have detections
                        if _fw_key not in st.session_state or \
                                (not st.session_state[_fw_key] and _auto_fw_list):
                            st.session_state[_fw_key] = [fw for fw in _auto_fw_list if fw in _fw_allowed]

                        compliance_frameworks = st.multiselect(
                            "Compliance Frameworks",
                            options=_fw_allowed,
                            help="Auto-suggested from detected categories & asset name. Adjust freely.",
                            key=_fw_key,
                        )
                        # ───────────────────────────────────────────────────────
                ...
                with tag_tab2:
                    if st.button("Apply Tags Immediately", key="btn_apply_tags", type="primary"):
                        ...
                        try:
                            _schema_for_apply = table_details['Schema'].iloc[0] if not table_details.empty else "TEST_DATA"
                        except Exception:
                            _schema_for_apply = "TEST_DATA"

                        # Guard: if selected_table already contains dots it may be fully-qualified
                        _tbl_parts = str(selected_table or "").split(".")
                        if len(_tbl_parts) >= 3:
                            full_obj_name = selected_table   # already DB.SCHEMA.TABLE
                        elif len(_tbl_parts) == 2:
                            full_obj_name = f"{self._get_active_database()}.{selected_table}"
                        else:
                            full_obj_name = f"{self._get_active_database()}.{_schema_for_apply}.{selected_table}"

                        if target_type == "Table":
                            # ── Step 1: Apply Snowflake tags directly (required) ──────────
                            try:
                                tagging_service.apply_tags_to_object(full_obj_name, "TABLE", tags_to_apply)
                                st.success(f"✅ Tags applied to **{full_obj_name}** successfully!")
                                st.balloons()
                            except Exception as tag_err:
                                st.error(f"❌ Failed to apply tags: {tag_err}")
                                st.caption(
                                    "Ensure the Snowflake role has `APPLY TAG` privilege on the object "
                                    "and that the object name is correct."
                                )
                                st.session_state['last_tag_error'] = str(tag_err)
                                st.stop()

                            # ── Step 2: Record workflow decision (best-effort) ────────────
                            try:
                                ...
                                pass  # Non-critical — tag was already applied

                            # ── Step 3: Masking-policy enforcement (best-effort) ──────────
                            try:
                                compliance_service.enforcement.process_pending_enforcements()
                            except Exception as enf_err:
                                st.caption(
                                    f"ℹ️ Tags applied. Masking policy enforcement skipped "
                                    f"(insufficient privilege or table not accessible for ALTER): "
                                    f"`{enf_err}`"
                                )
                            ...
                            if target_col:
                                try:
                                    ...
                                    st.success(f"✅ Tags applied to column **{target_col}** successfully!")
                                    st.balloons()
                                except Exception as col_err:
                                    st.error(f"❌ Failed to apply tags to column: {col_err}")
                                    st.session_state['last_tag_error'] = str(col_err)
                            else:
    ...
    def _init_local_embeddings(self) -> None:
        """
        Setup metadata and prepare for classification using cached governance metadata.
        This now loads metadata ONCE per session instead of per column.
        """
        # Check if metadata is already loaded from cache
        cached_metadata_key = "governance_metadata_cached"
        
        if cached_metadata_key in st.session_state:
            logger.debug("Using cached governance metadata (already loaded)")
            cached_data = st.session_state[cached_metadata_key]
            
            # Restore all metadata from cache
            self._category_metadata = cached_data.get('category_metadata', {})
            self._category_keywords = cached_data.get('category_keywords', {})
            self._category_patterns = cached_data.get('category_patterns', {})
            self._exclusion_patterns = cached_data.get('exclusion_patterns', {'exact': [], 'regex': []})
            self._policy_group_keywords = cached_data.get('policy_group_keywords', {})
            self._category_centroids = cached_data.get('category_centroids', {})
            self._category_tokens = cached_data.get('category_tokens', {})
            
            # Set embedder status
            self._embed_ready = cached_data.get('embed_ready', False)
            if cached_data.get('embedder_available') and not self._embedder:
                try:
                    self._embedder = SentenceTransformer('all-MiniLM-L6-v2') if SentenceTransformer else None
                    self._embed_backend = 'sentence-transformers' if self._embedder else 'none'
                except Exception:
                    self._embedder = None
                    self._embed_backend = 'none'
            
            # Update legacy structures for backward compatibility
            self._update_legacy_structures_from_cached_metadata(cached_data)
            self._view_based_rules_loaded = True
            return
        
        # Load metadata using the cached function (runs once per session)
        try:
            logger.info("Loading governance metadata (cached initialization)...")
            cached_data = load_governance_metadata_once()
            
            # Store in session state for future use
            st.session_state[cached_metadata_key] = cached_data
            ...
            logger.info("✓ Governance metadata loaded and cached successfully")
            ...
            self._exclusion_patterns = {'exact': [], 'regex': []}
            self._policy_group_keywords = {}
    ...
    def _update_legacy_structures_from_cached_metadata(self, cached_data: Dict[str, Any]) -> None:
        """
        Update legacy data structures from cached metadata for backward compatibility.
        This replaces the individual view loading with cached data.
        """
        try:
            # Update thresholds, weights, and other legacy structures
            ...
            for cat_name, metadata in cached_data.get('category_metadata', {}).items():
                # Update thresholds
                self._category_thresholds[cat_name] = metadata.get('detection_threshold', 0.5)
                self._category_default_thresholds[cat_name] = metadata.get('detection_threshold', 0.5)
                
                # Update weights
                self._category_weights[cat_name] = 1.0
                self._category_scoring_weights[cat_name] = {
                    'w_sem': metadata.get('weight_embedding', 0.60),
                    'w_kw': metadata.get('weight_keyword', 0.25),
                    'w_pat': metadata.get('weight_pattern', 0.15),
                }
                ...
                if policy_group:
                    self._policy_group_by_category[cat_name.upper()] = policy_group.upper()
                
                # Update category IDs
                if metadata.get('category_id'):
                    self._category_ids[cat_name] = metadata.get('category_id')
                
                # Update colors based on policy group
                pg = policy_group.upper()
                if pg == 'PII':
            ...
            # Update keyword and pattern metadata structures
            self._category_keyword_metadata = {}
            self._category_pattern_metadata = {}
            
            for cat_name, keywords in cached_data.get('category_keywords', {}).items():
            ...
            for cat_name, patterns in cached_data.get('category_patterns', {}).items():
            ...
            logger.debug("Updated legacy structures from cached metadata")
            
        except Exception as e:
            logger.warning(f"Failed to update legacy structures from cached metadata: {e}")
    ...
    def _display_classification_results(self, results: List[Dict[str, Any]]) -> None:
        ...
        for result in successful_results:
            ...
            if c_val <= 0:
                ...
                c_label = "🟠 Restricted (C2)"
            else:
                c_label = "🔒 Confidential (C3)"
    ...
    def detect_pii_native(self, table_fqn: str) -> Dict[str, Any]:
        """Detect PII data using the Snowflake inbuilt function EXTRACT_SEMANTIC_CATEGORIES."""
        # Snowflake function expects literal string
        query = f"SELECT EXTRACT_SEMANTIC_CATEGORIES('{table_fqn}') as RESULT"
        try:
            results = snowflake_connector.execute_query(query)
            # The result could be under 'RESULT' or 'result' key
            res_val = None
            if results:
                res_val = results[0].get('RESULT') or results[0].get('result')
            
            if res_val:
                import json
                if isinstance(res_val, str):
                    return json.loads(res_val)
                return res_val # Already a dict/list if connector handles it
            return {}
        except Exception as e:
            logger.error(f"Error in Snowflake native PII detection for {table_fqn}: {e}")
            raise
    ...
    # -------------------------------------------------------------------------
    # Compatibility Layer for UI Pages (Legacy API Support)
    # -------------------------------------------------------------------------

    def detect_sensitive_columns(self, database: str, schema_name: Optional[str] = None, table_name: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
        Optimized detection delegating to the unified classification pipeline.
        Supports both (database, schema, table) and (fully_qualified_name) signatures.
        """
        # Handle single FQN argument case
        if database and not schema_name and not table_name and "." in str(database):
            parts = str(database).split('.')
            if len(parts) == 3:
                database, schema_name, table_name = parts
            elif len(parts) == 2:
                # Assume DB from session/context if missing
                table_name = parts[1]
                schema_name = parts[0]
                database = getattr(self, "_active_db", "DATA_CLASSIFICATION_DB")
        
        # Determine schema if missing (fallback)
        if not schema_name and not table_name:
             return []

        # Default fallback results structure to prevent NoneType errors
        fallback = {'column_results': []}

        try:
            return self.get_bulk_suggestions(database, schema_name, table_name).get('column_results', [])
        except Exception:
            return []
    ...
    def get_bulk_suggestions(self, database: str, schema_name: str, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self._run_governance_driven_pipeline(database, [asset])
            ...
            logger.warning(f"Bulk suggestion failed for {database}.{schema_name}.{table_name}: {e}")
            return {'column_results': [], 'c': 1, 'i': 1, 'a': 1, 'Sensitivity': 'Internal'}

    def _suggest_cia_from_type(self, category: str) -> Dict[str, Any]:
        """Legacy helper for UI to get CIA suggestions."""
        info = self._get_category_policy_info(category)
        return {
            "C": info.get("c", 1),
            "I": info.get("i", 1),
            "A": info.get("a", 1),
            "label": info.get("label", "Internal")
        }
...
class AISensitiveDetectionService:
    ...
    def detect_sensitive_columns(self, database: str, schema_name: Optional[str] = None, table_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """Optimized detection delegating to the unified classification pipeline.
        Supports both (database, schema, table) and (fully_qualified_name) signatures.
        """
        # Handle single FQN argument case
    ...
    def run_native_pii_discovery(self, database_name: str, limit: Optional[int] = None) -> Dict[str, Any]:
        """
        Run Snowflake's native EXTRACT_SEMANTIC_CATEGORIES on all tables in the database.
        This reproduces the logic of the standalone 'scan_all_pii_native.py' script.
        """
        logger.info(f"Starting Native PII Discovery for database: {database_name}")
        ...
        try:
            query = f"""
                ...
                ORDER BY TABLE_SCHEMA, TABLE_NAME
            """
            tables = snowflake_connector.execute_query(query) or []
            if limit:
                tables = tables[:limit]
        except Exception as e:
            logger.error(f"Failed to fetch tables for PII scan: {e}")
        ...
        for table in tables:
            db = table.get('DATABASE_NAME')
            schema = table.get('SCHEMA_NAME')
            tbl = table.get('TABLE_NAME')
            fqn = f"{db}.{schema}.{tbl}"
            
            try:
                # 2. Run EXTRACT_SEMANTIC_CATEGORIES
                res = snowflake_connector.execute_query(f"SELECT EXTRACT_SEMANTIC_CATEGORIES('{fqn}') as RESULT")
                if not res or not res[0].get('RESULT'):
                    continue
                
                result_str = res[0].get('RESULT')
                try:
                    import json
                ...
                pii_cols = []
                for col_name, col_info in result_data.items():
                    privacy_cat = col_info.get('privacy_category', '')
                    semantic_cat = col_info.get('semantic_category', '')
                    
                    is_pii = (
                        privacy_cat in ('IDENTIFIER', 'QUASI_IDENTIFIER') or
                        semantic_cat in ('NAME', 'EMAIL', 'PHONE_NUMBER', 'SSN', 
                                        'CREDIT_CARD', 'ADDRESS', 'PERSON', 'AGE', 
                                        'GENDER', 'PASSPORT', 'DRIVERS_LICENSE')
                    )
                    ...
                    if is_pii:
                        pii_cols.append({
                            ...
                            'probability': col_info.get('extra_info', {}).get('probability', 0)
                        })
                
                # 4. Update Assets & Store Results
                has_pii = len(pii_cols) > 0
                
                # Update Asset PII Flag
                update_q = f"""
                    UPDATE {db}.DATA_CLASSIFICATION_GOVERNANCE.ASSETS
                    SET PII_RELEVANT = {has_pii},
                        LAST_MODIFIED_TIMESTAMP = CURRENT_TIMESTAMP()
                    WHERE DATABASE_NAME = '{db}' AND SCHEMA_NAME = '{schema}' AND ASSET_NAME = '{tbl}'
                """
                snowflake_connector.execute_non_query(update_q)
                ...
                if has_pii:
                    ...
                    for col in pii_cols:
                        merge_q = f"""
                        MERGE INTO {db}.DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_AI_RESULTS AS target
                        USING (SELECT 
                            '{db}' as D, '{schema}' as S, '{tbl}' as T, '{col['column_name']}' as C,
                            'PII' as CAT, '{col['semantic_category']}' as SEM,
                            {col['probability']} as P
                        ) AS source
                        ON target.DATABASE_NAME = source.D AND target.SCHEMA_NAME = source.S 
                        AND target.TABLE_NAME = source.T AND target.COLUMN_NAME = source.C
                        WHEN MATCHED THEN UPDATE SET FINAL_CONFIDENCE = source.P, UPDATED_AT = CURRENT_TIMESTAMP()
                        WHEN NOT MATCHED THEN INSERT (
                            DATABASE_NAME, SCHEMA_NAME, TABLE_NAME, COLUMN_NAME, 
                            AI_CATEGORY, SEMANTIC_CATEGORY, FINAL_CONFIDENCE, CREATED_AT, UPDATED_AT
                        ) VALUES (source.D, source.S, source.T, source.C, source.CAT, source.SEM, source.P, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP())
                        """
                        snowflake_connector.execute_non_query(merge_q)
                
                scanned += 1
                
            except Exception as e:
                logger.error(f"Error scanning {fqn}: {e}")
                errors += 1
...
# NOTE: Do NOT override DiscoveryService / discovery_service here; the real
# DiscoveryService class and its singleton instance are defined above (line 461)
# and used by other modules which rely on methods like mark_classified.
