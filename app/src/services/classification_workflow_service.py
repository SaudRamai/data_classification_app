...
class ClassificationWorkflowService:
    ...
    def _ensure_history_table(self, db: str) -> None:
        """Ensure the CLASSIFICATION_HISTORY table exists."""
        table = f"{db}.{SCHEMA}.CLASSIFICATION_HISTORY"
        try:
            # We follow the schema from schema_tables.sql
            self.connector.execute_non_query(f"""
                CREATE TABLE IF NOT EXISTS {table} (
            ...
            logger.error(f"Failed to ensure history table {table}: {e}")
    ...
    def record_decision(self, asset_full_name: str, decision_by: str, source: str, status: str, label: str, c: int, i: int, a: int, rationale: str, details: Optional[Dict[str, Any]] = None, database: Optional[str] = None, reviewer: Optional[str] = None) -> str:
        ...
        if details and isinstance(details, dict):
            # Extract from 'compliance' and 'special_category' lists if present
            c_vals = details.get("compliance") or []
            s_vals = details.get("special_category") or []
            
            # Combine all flags
            all_flags = []
            if isinstance(c_vals, list): all_flags.extend([str(x) for x in c_vals])
            elif c_vals: all_flags.append(str(c_vals))
            
            if isinstance(s_vals, list): all_flags.extend([str(x) for x in s_vals])
            elif s_vals: all_flags.append(str(s_vals))
            
            # Remove duplicates and empty strings
            all_flags = [f for f in sorted(list(set(all_flags))) if f.strip()]
            compliance_flags = ",".join(all_flags)
    ...
    def list_requests(self, status: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            sql = f"SELECT * FROM {table} {where} ORDER BY CREATED_AT DESC LIMIT {limit}"
            return self.connector.execute_query(sql) or []
        except Exception as e:
            logger.error(f"Failed to list requests: {e}")
            return []

    def submit_request(self, asset_full_name: str, proposed: Tuple[str, int, int, int], justification: str, requester: str, trigger_type: str = "MANUAL") -> str:
        ...
        p_cls, p_c, p_i, p_a = proposed
        
        # Get current state
        curr_rows = self.connector.execute_query(f"SELECT CLASSIFICATION_LABEL, CONFIDENTIALITY_LEVEL, INTEGRITY_LEVEL, AVAILABILITY_LEVEL FROM {db}.{SCHEMA}.ASSETS WHERE FULLY_QUALIFIED_NAME = %(f)s", {"f": asset_full_name})
        c_cls, c_c, c_i, c_a = "Unclassified", 0, 0, 0
        if curr_rows:
            r = curr_rows[0]
            c_cls = r.get("CLASSIFICATION_LABEL") or "Unclassified"
            def _p(v):
                if not v: return 0
                v = str(v).upper()
                if v.startswith(('C','I','A')):
                    try: return int(v[1:])
                    except: return 0
                try: return int(float(v))
                except: return 0
            c_c = _p(r.get("CONFIDENTIALITY_LEVEL"))
            c_i = _p(r.get("INTEGRITY_LEVEL"))
            c_a = _p(r.get("AVAILABILITY_LEVEL"))

        sql = f"""
            INSERT INTO {table} (ID, ASSET_FULL_NAME, PROPOSED_CLASSIFICATION, PROPOSED_C, PROPOSED_I, PROPOSED_A, 
                               CURRENT_CLASSIFICATION, CURRENT_C, CURRENT_I, CURRENT_A, JUSTIFICATION, STATUS, CREATED_BY, TRIGGER_TYPE)
            VALUES (%(id)s, %(full)s, %(pcls)s, %(pc)s, %(pi)s, %(pa)s, %(ccls)s, %(cc)s, %(ci)s, %(ca)s, %(just)s, 'Pending', %(by)s, %(trig)s)
        """
        self.connector.execute_non_query(sql, {
            "id": rid, "full": asset_full_name, "pcls": p_cls, "pc": p_c, "pi": p_i, "pa": p_a,
            "ccls": c_cls, "cc": c_c, "ci": c_i, "ca": c_a, "just": justification, "by": requester, "trig": trigger_type
        })
        return rid
