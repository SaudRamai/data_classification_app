...
-- Frameworks
INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.FRAMEWORKS (ID, NAME, VERSION, EFFECTIVE_DATE, NEXT_REVIEW_DATE, OWNER, CREATED_AT)
...
WHERE NOT EXISTS (
  SELECT 1 FROM DATA_CLASSIFICATION_GOVERNANCE.FRAMEWORKS f WHERE UPPER(f.NAME) = UPPER(s.NAME)
);

-- Policies (AI/NLP extracted minimal rules)
-- PII rules (GDPR/CCPA/DPDP)
INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.POLICIES (ID, FRAMEWORK, RULE_CODE, RULE_TEXT, CATEGORY, MIN_CONFIDENTIALITY, REQUIRE_MASKING, REQUIRE_ROW_ACCESS, CREATED_AT, SOURCE, DETAILS)
...
FROM DATA_CLASSIFICATION_GOVERNANCE.FRAMEWORKS fw
WHERE UPPER(fw.NAME) IN ('GDPR','CCPA','DPDP')
  AND NOT EXISTS (
    SELECT 1 FROM DATA_CLASSIFICATION_GOVERNANCE.POLICIES p WHERE UPPER(p.FRAMEWORK)=UPPER(fw.NAME) AND p.RULE_CODE='PII_MIN'
  );
...
WHERE UPPER(fw.NAME) = 'HIPAA'
  AND NOT EXISTS (
    SELECT 1 FROM DATA_CLASSIFICATION_GOVERNANCE.POLICIES p WHERE UPPER(p.FRAMEWORK)=UPPER(fw.NAME) AND p.RULE_CODE='PHI_MIN'
  );
...
WHERE UPPER(fw.NAME) = 'SOX'
  AND NOT EXISTS (
    SELECT 1 FROM DATA_CLASSIFICATION_GOVERNANCE.POLICIES p WHERE UPPER(p.FRAMEWORK)=UPPER(fw.NAME) AND p.RULE_CODE='FIN_MIN'
  );
...
WHERE UPPER(fw.NAME) = 'PCI-DSS'
  AND NOT EXISTS (
    SELECT 1 FROM DATA_CLASSIFICATION_GOVERNANCE.POLICIES p WHERE UPPER(p.FRAMEWORK)=UPPER(fw.NAME) AND p.RULE_CODE='PCI_MIN'
  );
