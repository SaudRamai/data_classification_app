...
CREATE OR REPLACE PROCEDURE DATA_CLASSIFICATION_GOVERNANCE.GEN_DYNAMIC_COMPLIANCE_REPORTS()
RETURNS STRING
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS
...
var insViol = `
INSERT INTO " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.VIOLATIONS
...
FROM APV a
JOIN " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.FRAMEWORKS fw ON 1=1
LEFT JOIN LATERAL FLATTEN(input => a.CATEGORIES) c ON TRUE
JOIN " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.POLICIES p
...
var insReports = `
INSERT INTO " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.COMPLIANCE_REPORTS
  ...
  OBJECT_CONSTRUCT(
    ...
    'compliant_assets', COUNT(DISTINCT CASE WHEN NOT (
      EXISTS (
        ...
        JOIN " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.POLICIES p2
    ...
    'non_compliant_assets', COUNT(DISTINCT CASE WHEN (
      EXISTS (
        ...
        JOIN " + DB + ".DATA_CLASSIFICATION_GOVERNANCE.POLICIES p3
...
CREATE OR REPLACE TASK DATA_CLASSIFICATION_GOVERNANCE.TASK_GEN_DYNAMIC_COMPLIANCE
  WAREHOUSE = <WAREHOUSE_NAME>
  SCHEDULE = 'USING CRON 0 2 * * * Asia/Kolkata'
AS
  CALL DATA_CLASSIFICATION_GOVERNANCE.GEN_DYNAMIC_COMPLIANCE_REPORTS();

-- To enable the task after setting the warehouse:
-- ALTER TASK DATA_CLASSIFICATION_GOVERNANCE.TASK_GEN_DYNAMIC_COMPLIANCE RESUME;
