    ...
    INPUT =>
        CASE
            WHEN C.CATEGORY_NAME = 'PII' THEN ARRAY_CONSTRUCT(
                ...
                'customer_id','tax_id','voter_id','military_id','alien_number', 'citizen_id',
                'ethnicity','religion','disability','gps','location','call_records',
                'video_call','income','credit_score','voice_print','fingerprint','biometric'
            )
            WHEN C.CATEGORY_NAME = 'FINANCIAL' THEN ARRAY_CONSTRUCT(
                ...
                'swift_code','tax_return','payment_history','business_registration'
            )
            WHEN C.CATEGORY_NAME = 'HEALTH' THEN ARRAY_CONSTRUCT(
                'diagnosis','mrn','disease','treatment',
                'doctor','prescription','medication','lab_result','hospital','insurance_id',
                'blood_group','vaccine','allergy','disability'
            )
            ...
            WHEN C.CATEGORY_NAME = 'SOC2' THEN ARRAY_CONSTRUCT(
                ...
                'two_factor','2fa','password_hash','security_question',
                'device_id','trade_secret','nda','confidential_agreement','encrypted_message'
            )
...
-- =====================================================
-- ✅ SEED: SENSITIVE_KEYWORDS (OVERRIDES)
-- =====================================================
-- High Priority PII Overrides (EXACT MATCH)
INSERT INTO SENSITIVE_KEYWORDS (
    KEYWORD_ID, CATEGORY_ID, KEYWORD_STRING, MATCH_TYPE, SENSITIVITY_WEIGHT, IS_ACTIVE, CREATED_BY
)
SELECT 
    UUID_STRING(), C.CATEGORY_ID, K.VALUE::STRING, 'EXACT', 0.95, TRUE, CURRENT_USER()
FROM SENSITIVITY_CATEGORIES C,
