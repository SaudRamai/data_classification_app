...
GRANT SELECT ON VIEW DATA_CLASSIFICATION_GOVERNANCE.VW_MY_CLASSIFICATION_TASKS TO APPLICATION ROLE app_admin;
    ...
    BEGIN
        ...
        -- 1. Create a staging table for discovery
        CREATE OR REPLACE TEMPORARY TABLE DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING (
            DB STRING, SCHEMA STRING, NAME STRING, ASSET_TYPE STRING, FQN STRING
        );

        -- 2. Discovery Loop across accessible databases
        CREATE OR REPLACE TEMPORARY TABLE DATA_CLASSIFICATION_GOVERNANCE.TEMP_DB_LIST (DB_NAME STRING);
        BEGIN
            EXECUTE IMMEDIATE 'SHOW DATABASES';
            INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DB_LIST (DB_NAME)
            SELECT "name" FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
            WHERE "name" NOT IN ('SNOWFLAKE', 'SNOWFLAKE_ASSET_METADATA');
        EXCEPTION WHEN OTHER THEN
            INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DB_LIST (DB_NAME) SELECT CURRENT_DATABASE();
        END;

        FOR db_rec IN (SELECT DB_NAME FROM DATA_CLASSIFICATION_GOVERNANCE.TEMP_DB_LIST) DO
            LET db_name := db_rec.DB_NAME;
            
            -- Tables
            BEGIN
                EXECUTE IMMEDIATE 'SHOW TABLES IN DATABASE ' || db_name;
                INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING
                SELECT "database_name", "schema_name", "name", 'TABLE', "database_name" || '.' || "schema_name" || '.' || "name"
                FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
                WHERE "schema_name" NOT IN ('INFORMATION_SCHEMA', 'DATA_CLASSIFICATION_GOVERNANCE');
            EXCEPTION WHEN OTHER THEN END;

            -- Views
            BEGIN
                EXECUTE IMMEDIATE 'SHOW VIEWS IN DATABASE ' || db_name;
                INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING
                SELECT "database_name", "schema_name", "name", 'VIEW', "database_name" || '.' || "schema_name" || '.' || "name"
                FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
                WHERE "schema_name" NOT IN ('INFORMATION_SCHEMA', 'DATA_CLASSIFICATION_GOVERNANCE');
            EXCEPTION WHEN OTHER THEN END;
            ...
            BEGIN
                ...
                INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING
                SELECT "database_name", "schema_name", SPLIT_PART("name", '(', 1), 'STORED_PROCEDURE', "database_name" || '.' || "schema_name" || '.' || "name"
                FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
                WHERE "schema_name" NOT IN ('INFORMATION_SCHEMA', 'DATA_CLASSIFICATION_GOVERNANCE');
            EXCEPTION WHEN OTHER THEN END;
            ...
            BEGIN
                ...
                INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING
                SELECT "database_name", "schema_name", SPLIT_PART("name", '(', 1), 'FUNCTION', "database_name" || '.' || "schema_name" || '.' || "name"
                FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
                WHERE "schema_name" NOT IN ('INFORMATION_SCHEMA', 'DATA_CLASSIFICATION_GOVERNANCE');
            EXCEPTION WHEN OTHER THEN END;
            ...
            BEGIN
                ...
                INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING
                SELECT "database_name", "schema_name", "name", 'STAGE', "database_name" || '.' || "schema_name" || '.' || "name"
                FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))
                WHERE "schema_name" NOT IN ('INFORMATION_SCHEMA', 'DATA_CLASSIFICATION_GOVERNANCE');
            EXCEPTION WHEN OTHER THEN END;
        END FOR;

        -- 3. Prepare Enriched Merged View
        CREATE OR REPLACE TEMPORARY TABLE DATA_CLASSIFICATION_GOVERNANCE.TEMP_MERGE_SOURCE AS
        SELECT 
            MD5(UPPER(TRIM(s.DB || '.' || s.SCHEMA || '.' || s.NAME))) as ASSET_ID,
            ...
            COALESCE(cd.CONFIDENTIALITY_LEVEL, 'C1') as CONFIDENTIALITY_LEVEL,
            COALESCE(cd.INTEGRITY_LEVEL, 'I1') as INTEGRITY_LEVEL,
            COALESCE(cd.AVAILABILITY_LEVEL, 'A1') as AVAILABILITY_LEVEL,
        ...
        LEFT JOIN (
            SELECT ASSET_FULL_NAME, LABEL, CONFIDENTIALITY_LEVEL, INTEGRITY_LEVEL, AVAILABILITY_LEVEL
            FROM DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_DECISIONS
            WHERE STATUS = 'APPROVED'
            QUALIFY ROW_NUMBER() OVER (PARTITION BY ASSET_FULL_NAME ORDER BY DECISION_TIMESTAMP DESC) = 1
        ...
        WHEN MATCHED AND (
            ...
            t.PII_RELEVANT != s.PII_RELEVANT OR
            t.SOX_RELEVANT != s.SOX_RELEVANT OR
            ...
            UPDATE SET 
                ...
                t.CLASSIFICATION_METHOD = s.CLASSIFICATION_METHOD,
        ...
        WHEN NOT MATCHED THEN
            INSERT (
                ...
                CLASSIFICATION_METHOD, CREATED_TIMESTAMP, LAST_MODIFIED_TIMESTAMP, RECORD_VERSION, CREATED_BY
            )
            VALUES (
                ...
                s.CLASSIFICATION_METHOD, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), 1, CURRENT_USER()
            );

        -- 5. Calculate Metrics
        SELECT COUNT(*) INTO :rows_inserted FROM DATA_CLASSIFICATION_GOVERNANCE.ASSETS WHERE CREATED_TIMESTAMP >= :ts_start AND RECORD_VERSION = 1;
        SELECT COUNT(*) INTO :rows_updated FROM DATA_CLASSIFICATION_GOVERNANCE.ASSETS WHERE LAST_MODIFIED_TIMESTAMP >= :ts_start AND RECORD_VERSION > 1;

        -- 6. Audit Log
        INSERT INTO DATA_CLASSIFICATION_GOVERNANCE.CLASSIFICATION_AUDIT (
            ACTION, RESOURCE_ID, USER_ID, NEW_STATE, REASON
        )
        VALUES (
            'MERGE_RUN', run_id, CURRENT_USER(), 
            OBJECT_CONSTRUCT(
                'status', 'success', 
            ...
            'Automated discovery and merge of assets.'
        );

        RETURN 'Success: Discovered ' || (SELECT COUNT(*) FROM DATA_CLASSIFICATION_GOVERNANCE.TEMP_DISCOVERY_STAGING) || ' objects. Managed: ' || :rows_inserted || ' new, ' || :rows_updated || ' updated. RunID: ' || run_id;
    END;
    $$;
    ...
    FROM asset_metrics am GROUP BY am.DATABASE_NAME, am.SCHEMA_NAME;
