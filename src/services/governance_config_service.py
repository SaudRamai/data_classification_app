...
class GovernanceConfigService:
    ...
    def _resolve_db_internal(self) -> str:
        ...
        # 3) Current DB Probe (Requires connection)
        try:
            ...
            logger.debug(f"GovConfig: Skip DB probe due to connection status: {e}")

        # 4) Global Probe (Requires connection)
        try:
            ...
            logger.debug(f"GovConfig: Skip Global probe due to connection status: {e}")
